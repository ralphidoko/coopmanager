                    ...
                    <div class="col-xs-12">
                        <div class="box box-primary" >
                            <div class="box-header">
                                @if(Auth::user()->user_role == 'Admin1' && $saving->status == 'Processing')
                                    ...
                                    @elseif(Auth::user()->user_role == 'Admin1' && $saving->status == 'Approved')
                                    <button style="font-weight: bold;font-size: 15px;" type="submit" id="reject_loan" value="reject_loan" class="btn btn-danger">Reject Debit</button>
                                    @elseif(Auth::user()->user_role == 'Admin1' && $saving->status == 'Rejected')
                                ...
                                @endif
                                @if(Auth::user()->user_role == 'Admin2' && $saving->status == 'Processing')
                                    <button style="font-weight: bold;font-size: 15px;" type="submit" id="sec_approve" value="sec_approved" class="btn btn-primary">Sec Approve Debit</button>
                                    <button style="font-weight: bold;font-size: 15px;" type="submit" id="reject_loan" value="reject_loan" class="btn btn-danger">Reject Debit</button>
                                        {{--  @elseif(Auth::user()->user_role == 'Admin2' && $saving->status == "Rejected")--}}
                                        {{--    <button style="font-weight: bold;font-size: 15px;" type="submit" disabled id="revoke_rejection" value="revoke_rejection" class="btn btn-bitbucket">Set to Draft</button>--}}
                                @endif
                                <div class="container-fluid pull-right" style=" display: inline-flex;">
                                    <ol class="breadcrumb breadcrumb-arrow" style="font-weight: bold;font-size: 15px;">
                                        @if($saving->sec_approve == "sec_approved")
                                            <li id="sa"><a hreff="#" style="color: green">Secretary Approved</a></li>
                                        @endif
                                        @if($saving->chair_approve == "chair_approved")
                                ...
                                <script>
                                    ...
                                    $( "#sec_approve" ).on( "click", function(e) {
                                        ...
                                        $.ajax({
                                            url: '{{URL::to('/handleAdminWithdrawalApproval')}}',
                                            type: 'POST',
                                            dataType: 'json',
                                            data: data,
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="box-body table-responsive">
                                                        <table class="table table-bordered table-striped"  style="border: 2px;font-size: medium">
                                                            <input type="hidden" value="{{$saving->id}}" id="saving_id" />
                                                            <input type="hidden" value="{{$saving->sec_approve}}" id="loan_sec_app" />
                                                            <input type="hidden" value="{{$saving->chair_approve}}" id="loan_chair_app" />
                                                            <input type="hidden" value="{{number_format($saving->amount_withdrawn,2)}}" id="withdrawn_amount" />
                                                            <thead>
                                                            ...
                                                            <tr>
                                                                <td style="width: 150px">
                                                                    <img src="{{asset('/storage/'. $saving->passport_url)}}" class="user-image" height="150" width="150">
                                                                </td>
                                                                <td style="padding-left: 10px; width:30% ! important;">
                                                                    <strong>Full Name:</strong>&#160;&#160;&#160;{{$saving->name}}<br/>
                                                                    <strong>Membership No:</strong>&#160;&#160;&#160;{{$saving->member_id}}<br/>
                                                                    <strong>Designation:</strong>&#160;&#160;&#160;{{$saving->designation}}<br/>
                                                                    <strong>Department:</strong>&#160;&#160;&#160;{{$saving->department}}<br/>
                                                                    <strong>Office Location:</strong>&#160;&#160;&#160;{{$saving->office_location}}<br/>
                                                                    <strong>Telephone:</strong>&#160;&#160;&#160;{{$saving->phone_no}}<br/>
                                                                </td>
                                                                <td>
                                                                    <strong>Account No:</strong>&#160;&#160;&#160;{{$saving->ippis_no}}<br/>
                                                                    <strong style="font-size: large; border: 1px;">Withdrawal Status:</strong>&#160;&#160;&#160;&#160;&#160; {{$saving->status}}
                                                                    @if($saving->status=='Approved')&#160;&#160;<i class="fa fa-check"  style="color: green;" aria-hidden="true"></i>@endif
                                                                    @if($saving->status=='Processing')&#160;&#160;<i class="fa fa-refresh fa-spin" style="color: orange;" aria-hidden="true"></i>@endif
                                                                    @if($saving->status=='Rejected')&#160;&#160;<i class="fa fa-times" style="color: red;" aria-hidden="true"></i>@endif<br/>
                                                                    <strong>Description:</strong>&#160;&#160;&#160;&#160;&#160;  {{$saving->description}}<br/>
                                                                    @if($saving->status =='Rejected')<strong style="font-size: large;">Reason For Rejection:</strong>&#160;&#160;&#160;&#160;&#160;{{$saving->reason_for_rejection}}<br/>@endif
                                                                    <strong style="font-size: large;">Withdrawal Amount:</strong>&#160;&#160;&#160;&#160;&#160;<del style="text-decoration-style: double">N</del>{{number_format($saving->amount_withdrawn,2)}}<br />
                                                                    @if($saving->status =='Approved')<strong style="font-size: large;">Account Balance:</strong>&#160;&#160;&#160;&#160;&#160;<del style="text-decoration-style: double">N</del>{{number_format($saving->balance,2)}}@endif
                                                                </td>
                                                                <td>
