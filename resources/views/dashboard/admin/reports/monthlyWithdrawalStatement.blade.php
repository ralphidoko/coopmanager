    ...
    <div class="pull-right" style="width: 100%; display: flex; justify-content: center; align-items: center;">
        <table id="saving">
            ...
            @foreach($monthlyDrawings as $monthlyDrawing)
                <tr id="s">
                    <td id="s">{{$counter++}}</td>
                    <td id="s">{{$monthlyDrawing->description}}</td>
                    <td id="s">{{date('M-Y',strtotime($monthlyDrawing->month_year))}}</td>
                    <td id="s" style="text-align: right">{{number_format($monthlyDrawing->amount_withdrawn,2)}}</td>
                </tr>
            @endforeach
            <tr id="figure" style="border: 1px solid #0a0a0a ! important;">
                <td colspan="2"><b>Total Withdrawals</b></td>
                <td colspan="2" style="text-align: right ! important;"><b>{{number_format($totalDrawings,2)}}</b></td>
            </tr>
            </tbody>
        </table>
    </div>
