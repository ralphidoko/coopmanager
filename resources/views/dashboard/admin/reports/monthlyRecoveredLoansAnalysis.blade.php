        ...
        <table>
            <tr>
                ...
                <th>
                    ...
                    <p style="margin-top: -17px; font-weight:bolder; font-size: 15px; background-color: black;color: #FFFFFF; ">MONTHLY RECOVERED LOAN FOR THE PERIOD  {{date('d-m-Y',strtotime($startDate))}} - {{date('d-m-Y',strtotime($endDate))}}</p>
                </th>
            </tr>
        </table>
    </div>
        ...
        <table id="saving">
            ...
            @foreach($monthlyRecoveredLoans as $monthlyRecoveredLoan)
                <tr id="s">
                    ...
                    <td id="s">{{date('M-Y',strtotime($monthlyRecoveredLoan->month_year))}}</td>
                    <td id="s" style="text-align: right"><del style="text-decoration-style: double">N</del>{{number_format($monthlyRecoveredLoan->amount_recovered,2)}}</td>
                </tr>
            @endforeach
            <tr id="figure" style="border: 1px solid #0a0a0a ! important;">
                <td colspan="2"><b>Total Recovered Loans</b></td>
                <td colspan="2" style="text-align: right ! important;"><b><del style="text-decoration-style: double">N</del>{{number_format($totalRecoveredLoan,2)}}</b></td>
            </tr>
            </tbody>
        </table>
    </div>
